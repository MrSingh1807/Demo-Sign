    ...
    override fun onTouch(view: View?, event: MotionEvent?): Boolean {
        ...
        when (action) {
            MotionEvent.ACTION_POINTER_DOWN -> {
                ...
                logDebug("Action: ACTION_POINTER_DOWN")
                zoomAndRotateSticker(rootView, event)
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                val zoomFactor = if (distanceFromCenter > 0) 1.1f else 0.9f
                rootView.setScaleX(startScale * zoomFactor)
                rootView.setScaleY(startScale * zoomFactor)

                // Calculate rotation angle
                val angle =
                    Math.atan2(deltaY.toDouble(), deltaX.toDouble()) * 180f / Math.PI
