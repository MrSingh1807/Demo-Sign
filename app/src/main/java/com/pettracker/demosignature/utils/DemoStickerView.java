...
public class StickerView extends FrameLayout {
    ...
    public boolean remove(@Nullable Sticker sticker) {
        if (stickers.contains(sticker)) {
            ...
            redoList.push(sticker); //todo: MrSingh modified
            if (onStickerOperationListener != null) {
                onStickerOperationListener.onStickerDeleted(sticker, stickers);
            }
    ...
    protected void addStickerImmediately(@NonNull Sticker sticker, @Sticker.Position int position) {
        ...
        undoList.push(sticker); //todo: MrSingh Modified
        if (!isRedo) redoList.clear();  //todo: MrSingh Modified
        if (onStickerOperationListener != null) {
    ...
    /********todo: MrSingh modified  ********/
    public final Stack<Sticker> undoList = new Stack<>();
    public final Stack<Sticker> redoList = new Stack<>();
    ...
    public void undoSticker() {
        if (!undoList.isEmpty()) {
            isUndo = true;
            Sticker undoSticker = undoList.pop();

            remove(undoSticker);
//            addStickerUndoRedo(undoSticker);
        }
    }
    ...
    public void redoSticker() {
        if (!redoList.isEmpty()) {
            isRedo = true;
            Sticker redoSticker = undoList.pop();
            undoList.push(redoSticker);

            addStickerUndoRedo(redoSticker);
        }
    }

    public void addStickerUndoRedo(@NonNull Sticker sticker) {
        handlingSticker = sticker;
        stickers.add(sticker);
        if (onStickerOperationListener != null) {
            onStickerOperationListener.onStickerAdded(sticker);
    ...
    public interface OnStickerOperationListener {
        ...
        void onStickerDeleted(@NonNull Sticker sticker, List<Sticker> stickers);
